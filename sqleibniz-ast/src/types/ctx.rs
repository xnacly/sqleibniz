...
impl HookContext {
    ...
    pub fn token(token: &super::Token) -> Self {
        let (kind, content) = match &token.ttype {
            ...
            Type::Plus => ("Plus", Some("+".into())),
            Type::Minus => ("Minus", Some("-".into())),
            Type::Asterisk => ("Asterisk", Some("*".into())),
            Type::Slash => ("Slash", Some("/".into())),
            Type::Semicolon => ("Semicolon", Some(";".into())),
            Type::Percent => ("Percent", Some("%".into())),
            Type::Comma => ("Comma", Some(",".into())),
            Type::Equal => ("Equal", Some("=".into())),
            Type::EqualEqual => ("EqualEqual", Some("==".into())),
            Type::Less => ("Less", Some("<".into())),
            Type::LessEqual => ("LessEqual", Some("<=".into())),
            Type::Greater => ("Greater", Some(">".into())),
            Type::GreaterEqual => ("GreaterEqual", Some(">=".into())),
            Type::NotEqual => ("NotEqual", Some("!=".into())),
            Type::PipePipe => ("PipePipe", Some("||".into())),
            Type::Ampersand => ("Ampersand", Some("&".into())),
            Type::Pipe => ("Pipe", Some("|".into())),
            Type::Tilde => ("Tilde", Some("~".into())),
            Type::ShiftLeft => ("ShiftLeft", Some("<<".into())),
            Type::ShiftRight => ("ShiftRight", Some(">>".into())),
            Type::Arrow => ("Arrow", Some("->".into())),
            Type::ArrowArrow => ("ArrowArrow", Some("->>".into())),
