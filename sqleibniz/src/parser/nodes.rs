...
sqleibniz parses the generic PRAGMA statement shape first, then analyses known SQLite PRAGMAs against the invocation forms and value categories documented on the SQLite PRAGMA page. Unknown or extension PRAGMAs stay parseable and do not produce diagnostics. Optional schema prefixes are ignored for validation, so `PRAGMA main.wal_checkpoint(FULL);` is validated as `wal_checkpoint`.
